...
async function hashPassword(plain) {
    return argon2_1.default.hash(plain, ARGON_OPTIONS);
}
async function upsertUser(pool, params) {
    await pool.query(`insert into users (user_id, email, display_name, role, password, account_onboarding_complete, created_at, updated_at)
...
async function upsertPatientProfile(pool, patientId, mobile, preferred, emergencyName) {
    await pool.query(`insert into patient_profiles (
     ...
     on conflict (user_id) do update set
       ...
       updated_at = now()`, [patientId, mobile, preferred, emergencyName]);
}
async function upsertPsychologistProfile(pool, psychologistId, registrationNumber, providerNumber, specialties, status) {
    await pool.query(`insert into psychologist_profiles (user_id, registration_number, provider_number, specialties, status, updated_at)
     ...
     on conflict (user_id) do update set
       ...
       updated_at = now()`, [psychologistId, registrationNumber, providerNumber, specialties, status]);
}
async function seed() {
    ...
    for (const psychologist of psychologists) {
        ...
        await upsertPsychologistProfile(pool, psychologist.userId, psychologist.registrationNumber, psychologist.providerNumber, psychologist.specialties, psychologist.status);
    }
    const patients = [
        { userId: "user_patient_qa_001", email: "qa.patient1@clink.test", displayName: "QA Patient Active", preferred: "email" },
        { userId: "user_patient_qa_002", email: "qa.patient2@clink.test", displayName: "QA Patient Soft Deleted", preferred: "sms" },
        { userId: "user_patient_qa_003", email: "qa.patient3@clink.test", displayName: "QA Patient Legal Hold", preferred: "phone" },
        { userId: "user_patient_qa_004", email: "qa.patient4@clink.test", displayName: "QA Patient Purge Pending", preferred: "email" },
        { userId: "user_patient_qa_005", email: "qa.patient5@clink.test", displayName: "QA Patient High Risk", preferred: "sms" },
        { userId: "user_patient_qa_006", email: "qa.patient6@clink.test", displayName: "QA Patient Corrections", preferred: "email" },
    ];
    ...
    await pool.query(`update users set
     ...
     where user_id = $1`, ["user_patient_qa_002", iso(-30), iso(365 * 7 - 30)]);
    await pool.query(`update users set
     ...
     where user_id = $1`, ["user_patient_qa_003", iso(-5)]);
    await pool.query(`update users set
      deleted_at = $2::timestamptz,
      ...
      retention_until = $3::timestamptz,
      last_interaction_at = $2::timestamptz
     where user_id = $1`, ["user_patient_qa_004", iso(-(365 * 7 + 2)), iso(-2)]);
    await pool.query(`insert into patient_data_requests (
    ...
    on conflict (request_id) do update set
      ...
      updated_at = now()`, [iso(14), iso(-3), iso(10), iso(-1)]);
    await pool.query(`insert into referral_documents (
    ...
    on conflict (document_id) do update set
      ...
      due_at = excluded.due_at`, [iso(-20).slice(0, 10), iso(-8), iso(6), iso(-2)]);
    await pool.query(`insert into security_incidents (
    ...
    on conflict (incident_id) do update set
      ...
      resolution_notes = excluded.resolution_notes,
      updated_at = now(),
      closed_at = excluded.closed_at`, [iso(-7), iso(-1)]);
    await pool.query(`insert into patient_consents (consent_id, user_id, policy_version, accepted_at, withdrawn_at, withdrawal_reason)
     values
     ...
     on conflict (consent_id) do update set
       ...
       withdrawal_reason = excluded.withdrawal_reason`, [iso(-12), iso(-2)]);
    await pool.query(`insert into booking_requests (
    ...
    on conflict (booking_request_id) do update set
      ...
      updated_at = now()`, [iso(2).slice(0, 10), iso(-1)]);
...
//# sourceMappingURL=seed-rich-test-data.js.map